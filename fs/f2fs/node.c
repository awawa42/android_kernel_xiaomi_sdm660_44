...
int f2fs_get_dnode_of_data(struct dnode_of_data *dn, pgoff_t index, int mode)
{
	...
	for (i = 1; i <= level; i++) {
		...
		if (nids[i] && nids[i] == dn->inode->i_ino) {
			err = -EINVAL;
			printk(KERN_ERR "F2FS-fs (%s): inode mapping table is corrupted, "
			"run fsck to fix it, ino:%lu, nid:%u, level:%d, offset:%d\n",
			sbi->sb->s_id, dn->inode->i_ino, nids[i], level, offset[level]);
			set_sbi_flag(sbi, SBI_NEED_FSCK);
			goto release_pages;
		}
